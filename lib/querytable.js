var Q, QueryTable, old_console_log, prefix, replace_char, sqlbuilder, toType, _;
...
QueryTable = (function() {
  ...
  QueryTable.prototype.toSQL = function() {
    var columnName, f, field, i, k, name, operator, operators, opkey, sql, str, v, _i, _j, _k, _l, _len, _len1, _len2, _len3, _len4, _m, _ref, _ref1, _ref2, _ref3, _ref4, _ref5, _ref6, _ref7;
    if (this._queryType === 'find') {
      ...
      for (_l = 0, _len3 = _ref5.length; _l < _len3; _l++) {
        ...
        sql = sql.set(f.column, (_ref6 = f.value) != null ? _ref6 : null);
      }
  ...
  QueryTable.prototype.query = function() {
    var cacheKey, cacheToInstance, dbToInstance, deferred, instanceToCache, self, sql, _ref;
    self = this;
    deferred = Q.defer();
    sql = self.toSQL();
    self.db.debug = this._debug;
    cacheToInstance = function(rows) {
      var field, instances, name, row, _i, _len, _ref, _ref1;
      self.debug('[dbcover] Found data in cache.');
      instances = [];
      _ref = JSON.parse(rows);
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        row = _ref[_i];
        _ref1 = self.nameToField;
        for (name in _ref1) {
          field = _ref1[name];
    ...
    if (self._queryType === 'find') {
      ...
      if (self.cache) {
        ...
        self.db.query(sql, (_ref = self._condition) != null ? _ref : self._args, function(err, rows) {
          if (err != null) {
            return deferred.reject(err);
          } else {
